...
class Producer(AMQPMixin):
    ...
    def _ensure_exchange(self, exchange_name, *, exchange_kwargs):
        async
        with self._ensure_exchange_lock:
    ...
    def _connect(self):
        ...
        with self._connect_lock:
            if not self._connected:
                await
                super()._connect(self.amqp_url, **self.amqp_kwargs)
    ...
    def exchange_declare(self, exchange_name, *, exchange_kwargs=None):
    ...
    def _ensure_queue_bind(self, queue_name, exchange_name, routing_key=''):
        async
        with self._bind_queue_lock:
    ...
    def publish(
        ...
        try:
            ...
            await
            self._ensure_queue_bind(queue_name, exchange_name, routing_key)
    ...
    def queue_purge(self, queue_name, **kwargs):
        try:
            ...
            await
            self._queue_purge(queue_name, **kwargs)
        except:  # noqa
            await
            self._disconnect()
            raise
    ...
    def queue_unbind(self, queue_name, exchange_name, routing_key, **kwargs):
        try:
            ...
            await
            self._queue_unbind(queue_name, exchange_name, routing_key, **kwargs)
        except:  # noqa
            await
            self._disconnect()
            raise
    ...
    def queue_delete(self, queue_name, **kwargs):
        try:
            ...
            await
            self._queue_delete(queue_name, **kwargs)
        except:  # noqa
            await
            self._disconnect()
            raise
    ...
    def exchange_delete(self, exchange_name, **kwargs):
        try:
            ...
            await
            self._exchange_delete(exchange_name, **kwargs)
        except:  # noqa
            await
            self._disconnect()
            raise
